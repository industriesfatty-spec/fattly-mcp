...
const NO_KEY =
  "No API key. Set FATTLY_API_KEY (create a key at fattly.app → Dashboard → API keys) or run: fattly login";

// Readable error for a failed authenticated call. 401 = key missing/invalid/revoked.
function httpError(res, data) {
  if (res.status === 401) {
    return new Error(
      "Invalid or revoked API key (HTTP 401). Check FATTLY_API_KEY — keys start with gpx_live_ " +
        "and are managed at fattly.app → Dashboard → API keys."
    );
  }
  return new Error(data.error || `Server error (HTTP ${res.status}).`);
}
